...
const sessions = [
  { device: 'Chrome · Windows', loc: 'Balikpapan, ID', when: 'Aktif sekarang', current: true },
  { device: 'Safari · iPhone 14', loc: 'Balikpapan, ID', when: '2 hari lalu', current: false },
]
...
<template>
  ...
  <div class="rounded-2xl border border-border-card bg-white px-5 py-2">
    <div v-for="s in sessions" :key="s.device" class="flex items-center gap-3.5 border-b border-[#F4EFE4] py-3.5 last:border-b-0">
      <div class="flex h-[38px] w-[38px] flex-none items-center justify-center rounded-[10px] bg-[#EEF2F7] text-[17px]">🖥️</div>
